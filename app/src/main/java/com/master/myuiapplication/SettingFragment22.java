 ...
 * {@link SettingFragment22.OnFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link SettingFragment22#newInstance} factory method to
 * create an instance of this fragment.
 */
public class SettingFragment22 extends Fragment {
    // TODO: Rename parameter arguments, choose names that match
    // the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";
    private static final String ARG_BRUSH_NAME = "brush_name";
    private static final String TAG = "SettingFragment22";
    ...
    private Button btnDel; //DELETE button for inactive timer
...
//    public static ArrayList<Timers> inactiveTimes = null;//This is store all the start & end times
    ...
    private List iEnabledTimer;
    private boolean[] mCheckedTimer;

    public SettingFragment22() {
    ...
    public static SettingFragment22 newInstance(String brush_name, String param2) {
        SettingFragment22 fragment = new SettingFragment22();
...
//        setView = inflater.inflate(R.layout.fragment_setting, container, false);
        setView = inflater.inflate(R.layout.fragment_setting22, container, false);
        ...
        btnDel = (Button) setView.findViewById(R.id.btnDelete2);
...
//                    showEndDateTimePickerDialog(v);
        ...
        btnDel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(!containsSet(mCheckedTimer)){
                    Log.d(TAG, "No Timer selected. Please check at least ONE ! ");
                    showShortToast("No Timer selected. Please check at least ONE ! ");
                } else {
//                    Log.d(TAG, "Timer selected ! ");
                    for (int i = mTimerAdapter.getCount()-1; i>=0; i--) {
                        // need to start delete from the last,
                        // otherwise the count & position get changed on first delete !
                        // if it is checked, delete it !
//                        Log.d(TAG, "   " +i +"  " +mCheckedTimer[i]);
                        if(mCheckedTimer[i]) {
                            Log.d(TAG, " calling deleteTimer " + i);
                            deleteTimer(i);
                        }
                    }
                }
        ...
        Log.d(TAG, " getCurrentTime() hours " +hours +" min " +minutes);
        // getCurrentTime() hours 4: min 7  or  getCurrentTime() hours 16 min 07   depending on phone system settings
        Log.d(TAG, " FULL " +SimpleDateFormat.getTimeInstance(SimpleDateFormat.FULL).format(new Date())
                // 5:25:32 PM Irish Standard Time
    ...
    boolean containsSet(boolean[] myBooleanArray) {
//        for (boolean value : myBooleanArray) {
// not parsing the whole array because it has size MAX_NUM_INACTIVE_TIMERS
        for (int i = 0; (i<mTimerAdapter.getCount()); i++) {
            if (myBooleanArray[i]) {
                return true;
            }
        }
...
*/
                    ...
                    iEnabledTimer.add(inactiveEntity.getIsEnabled());
...
//        inactiveValue.setBrushName(ARG_BRUSH_NAME);
        ...
        inactiveValue.setIsEnabled(true);   // when a new timer is added, by default, is Enabled.
...
//        if(MainActivity.inactiveTimes.size() == 0) {
        if(mTimerAdapter.getCount() == 0) {
...
//            mCheckedTimer = new boolean[getCount()];
            mCheckedTimer = new boolean[MAX_NUM_INACTIVE_TIMERS];
            // set to MAX_NUM_INACTIVE_TIMERS to avoid OutOfBounds
            // always have to check this to bound by inactiveTimes.size() ==> getCount()

            iEnabledTimer = new ArrayList();
        }
        ...
        public void removeItem(int index) {
            ...
            iEnabledTimer.remove(index);
            mCheckedTimer[index] = false;   // better to reset to false here !
            notifyDataSetChanged();
        }
        ...
        public void clearAllItems() {
            ...
            iEnabledTimer.clear();
            Arrays.fill(mCheckedTimer, false);  // reset all to false here !
            notifyDataSetChanged();
        }
...
*/
            View row = inflater.inflate(R.layout.single_row_timers22, parent,
                    false); // contains a ref to Relative Layout
...
//            ImageView image = (ImageView) row.findViewById(R.id.imgViewClock);
            ...
            Switch timerSwitch =  (Switch) row.findViewById(R.id.switch_timer);
...
/*
            timerSwitch.setOnClickListener(new View.OnClickListener() {
                @Override public void onClick(View v) {
                    switchTimerToggle(position);
                }
            });
*/
            }

            if(iEnabledTimer.size() > 0) {  // otherwise it can be zero before fetching from DB completed.
                timerSwitch.setChecked((Boolean) iEnabledTimer.get(position));
                // default CHECKED / UNCHECKED depends on how it was, as read from the DB
            }

            timerSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    // isChecked will be true if the switch is ON
                    // "position" of 1st element is '0', 2nd element is '1', and so on
                    iEnabledTimer.set(position, isChecked);
                    switchTimerToggle(position, isChecked);
                    Log.d(TAG, " onCheckedChanged position " +position
                            +" isChecked " +isChecked +" iEnabledTimer " +iEnabledTimer);
                }
            });

            CheckBox cBox = (CheckBox) row.findViewById(R.id.checkBoxTimer);
            cBox.setTag(position); // set the tag so we can identify the correct row in the listener
            cBox.setChecked(mCheckedTimer[position]); // set the status as we stored it
//            cBox.setOnCheckedChangeListener(mCheckListener); // set the listener
            cBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    mCheckedTimer[(Integer)buttonView.getTag()] = isChecked; // get the tag so we know the row and store the status
                    Log.d(TAG, " onCheckedChanged  mCheckedTimer  " +Arrays.toString(mCheckedTimer));
                    // e.g., 4 items in list => onCheckedChanged mCheckedTimer [false, false, true, true]
                }
            });
...
//        MainActivity.inactiveTimes.remove(timerNumber);
        ...
        showShortToast( "Successfully Deleted Inactive Timer " + (timerNumber + 1));
//        updateTimerList();
    ...
    //When we ON / OFF switches, we go in here, arg is provide which timer we are deleting
    public void switchTimerToggle(int timerNumber, boolean isOnOff) {
...
//         need to update DB
        ...
        inactiveValue.setBrushName(brushName);
        inactiveValue.setInactiveTime(mTimerAdapter.getItem(timerNumber));
        inactiveValue.setIsEnabled(isOnOff);
        mBrushDataViewModel.insertInactive(inactiveValue);
// for UPDATE, doing INSERT; because it is OnConflictStrategy.REPLACE

// Should not remove this timer from DB, but should be sync with Brush BT

        showShortToast( "Successfully updated Inactive Timer " +(timerNumber + 1) +" with " +isOnOff);
        Log.d( TAG, "Successfully updated Inactive Timer " +(timerNumber + 1) +" with " +isOnOff);
//        updateTimerList();
